...
- **`status`** may still be `READY` immediately after confirm — the transition to `SENT` happens asynchronously. Use the [Query](./p2p-query) endpoint to poll for the updated status.
